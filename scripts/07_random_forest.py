...
RESULTS_DIR = os.path.join(BASE_RESULTS_DIR, '07_random_forest')
os.makedirs(RESULTS_DIR, exist_ok=True)

DATA_DIR = 'data_crudos'

print("\n" + "="*80)
print("🌳 RANDOM FOREST - LEAVE-ONE-LOCATION-OUT CROSS-VALIDATION")
print("="*80 + "\n")
...
def entrenar_fold(X_train, y_train, X_test, y_test):
    """Entrena Random Forest en un fold y devuelve métricas"""
    ...
    model_x = RandomForestRegressor(**params)
...
def main():
    ...
    df.to_csv(os.path.join(RESULTS_DIR, 'random_forest_results.csv'), index=False)
    
    # Crear visualizaciones
    fig, ax = plt.subplots(2, 2, figsize=(14, 10), dpi=300)
    
    # Panel 1: RMSE por fold
    ax[0, 0].bar(range(1, len(archivos)+1), df['rmse'], color='#2ECC71', edgecolor='black', linewidth=1.5)
    ...
    ax[0, 0].set_title('RMSE por Ubicación - Random Forest', fontsize=12, fontweight='bold')
    ax[0, 0].legend(fontsize=10)
    ax[0, 0].grid(True, alpha=0.3, axis='y')
    ax[0, 0].set_xticks(range(1, len(archivos)+1))
    
    # Panel 2: MAE por fold
    ax[0, 1].bar(range(1, len(archivos)+1), df['mae'], color='#F39C12', edgecolor='black', linewidth=1.5)
    ...
    ax[0, 1].set_title('MAE por Ubicación - Random Forest', fontsize=12, fontweight='bold')
    ...
    ax[1, 0].scatter([1]*len(df['rmse']), df['rmse'], color='#2ECC71', alpha=0.6, s=100, label='RMSE')
    ax[1, 0].scatter([2]*len(df['mae']), df['mae'], color='#F39C12', alpha=0.6, s=100, label='MAE')
    ax[1, 0].set_ylabel('Error (m)', fontsize=11, fontweight='bold')
    ax[1, 0].set_title('Distribución de Errores - Random Forest', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, 'random_forest_results.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, 'random_forest_tabla_detallada.png'), dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"✅ Gráficos guardados en {RESULTS_DIR}")
    print(f"✓ random_forest_results.csv")
    print(f"✓ random_forest_results.png")
    print(f"✓ random_forest_tabla_detallada.png")
    print("\n" + "="*80)
